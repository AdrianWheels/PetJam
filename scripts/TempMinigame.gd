...
func start_trial(config: TrialConfig) -> void:
        ...
        var item_label: String = String(config.get_parameter(&"label", "Forja"))
        var hardness_val: float = clamp(float(config.get_parameter(&"hardness", 0.5)), 0.0, 1.0)
        var temp_window: float = float(config.get_parameter(&"temp_window_base", 80))
        var precision_val: float = clamp(float(config.get_parameter(&"precision", 0.0)), 0.0, 1.0)
        _pending_blueprint = {
                "name": item_label,
                "hardness": hardness_val,
                "temp_window_base": temp_window,
...
func finish(ok):
        ...
        complete_trial(res)
        var qc: Dictionary = res.details.get("quality_counts", {"Perfect":0, "Bien":0, "Regular":0, "Miss":0})
