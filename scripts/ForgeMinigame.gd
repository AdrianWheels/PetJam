...
func _finish_attempt():
        ...
        var score: float = round(ratio * _max_score)
        var success: bool = diff <= _tolerance
...
func _draw():
        ...
        var tolerance_px: float = bar_rect.size.x * _tolerance
        ...
        var progress_x: float = bar_rect.position.x + clamp(_progress, 0.0, 1.0) * bar_rect.size.x
